...
export default function AboutScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: MS.color.cream }} contentContainerStyle={{ paddingTop: insets.top + 12, paddingHorizontal: 18, paddingBottom: 40 }}>
      <ScreenHeader eyebrow="University pilot" title="About MindSHED" description="A gentle wellbeing companion being designed with university students." />
      ...
      <SettingsSection label="Information">
        <SettingsRow icon="file-text" title="Privacy policy" detail="Data use, storage and your choices" onPress={() => router.push({ pathname: '/legal', params: { section: 'privacy' } })} />
        ...
        <SettingsRow icon="mail" title="Contact the MindSHED team" detail="App support, privacy and research contacts" onPress={() => router.push({ pathname: '/legal', params: { section: 'contact' } })} last />
      </SettingsSection>
      <Body size={10.5} color={MS.color.faint} style={{ textAlign: 'center', marginTop: 22 }}>MindSHED · pilot build {version}</Body>
    </ScrollView>
  );
}
