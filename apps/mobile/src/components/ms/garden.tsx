...
// Sky (sun + clouds) pins to the top, ground pins to the bottom, so the
// scene works at any container height (sheet open or collapsed).
const FLOWER_SPOTS = [
  { x: 52, y: 226 },
  { x: 78, y: 234 },
  { x: 252, y: 230 },
...
export function Garden({ flowers }: { flowers: number }) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: MS.color.sky, overflow: 'hidden' }}>
      <Svg
        style={{ position: 'absolute', top: 0, left: 0, width: '100%', aspectRatio: 340 / 110 }}
        viewBox="0 0 340 110">
        <Circle cx="288" cy="52" r="20" fill={MS.color.yellow} stroke={ink} strokeWidth="2.5" />
        <Ellipse cx="66" cy="54" rx="26" ry="11" fill="#FFFFFF" stroke={ink} strokeWidth="2" />
        <Ellipse cx="90" cy="46" rx="18" ry="8" fill="#FFFFFF" stroke={ink} strokeWidth="2" />
        <Ellipse cx="196" cy="78" rx="20" ry="9" fill="#FFFFFF" stroke={ink} strokeWidth="2" />
      </Svg>
      <Svg
        style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', aspectRatio: 340 / 270 }}
        viewBox="0 20 340 270">
        <Path
